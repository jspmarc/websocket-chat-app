...
const Auth: NextPage = () => {
	...
	const submitHandler = async (e: FormEvent<HTMLFormElement>) => {
		e.preventDefault();
		const isLogin =
			(e.currentTarget.querySelector('#type') as HTMLInputElement).value ===
			'login';
		...
		<div className="flex justify-between min-h-screen w-[100vw]">
			<div className="bg-gray-500 flex flex-col items-center justify-center m-auto pb-12 px-12 rounded-xl w-96">
				<div className="grid grid-cols-2 mb-8 w-full">
					<button
						className={`${currentTabLogin ? 'bg-black' : ''} rounded-b-xl`}
						onClick={() => setCurrentTabLogin(true)}
					>
						Login
					</button>
					<button
						className={`${currentTabLogin ? '' : 'bg-black'} rounded-b-xl`}
						onClick={() => setCurrentTabLogin(false)}
					>
						Register
					</button>
				</div>
				{(currentTabLogin && (
					...
					>
						...
						<button className="bg-black py-2 rounded-xl" type="submit">
							Login
						</button>

...
const Home: NextPage = ({
	...
	useEffect(() => {
		fetch('/api/users/decode', {
			...
			.then((e) => e.json())
			.then((decoded) => {
				for (const k in decoded) window.localStorage.setItem(k, decoded[k]);
			});
	...
	const onChange = (e: FormEvent<HTMLInputElement>) => {
		if (e.currentTarget.value.length <= 128) setInput(e.currentTarget.value);
	};
	...
	return (
		<div className="flex flex-col gap-4 h-[80vh] items-center m-auto min-h-screen overflow-hidden p-12 w-[80vw]">
			...
			>
				...
				/>
				<button type="submit">
					Send <span className="text-3xl -translate-y-8 relative">&gt;</span>
				</button>
			</form>
